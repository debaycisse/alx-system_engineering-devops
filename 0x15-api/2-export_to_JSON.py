...
def store_todos_in_csv(userId, username, data_list, f_path):
    """stores a given user's todo in a given json file

    Args:
        userId - the id of the given user
        username - the username of the given user
        data_list - the list of tasks of a given user
        f_path - path to the json file
    """
    obj = {f"{userId}": []}
    for task_obj in data_list:
        obj[userId].append(
            {"task": task_obj.get('title'),
             "completed": task_obj.get('completed'),
             "username": username}
        )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        store_todos_in_csv(sys.argv[1], emp_username, todo_res, file_path)
